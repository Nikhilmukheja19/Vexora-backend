...
export const createOrder = async (req, res, next) => {
  try {
    const { items, shippingAddress, paymentMethod = 'cod', notes, businessSlug } = req.body;
    ...
    for (const item of items) {
      ...
      if (!product.isActive) return errorResponse(res, `Product unavailable: ${product.name}`, 400);
...
export const getCustomerOrders = async (req, res, next) => {
  try {
    ...
    successResponse(res, { orders, pagination: { page: Number(page), limit: Number(limit), total, pages: Math.ceil(total / limit) } });
  } catch (error) {
    next(error);
  }
};
